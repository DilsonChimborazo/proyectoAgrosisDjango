...
// 🌐 WebSocket Servers
const char* websocket_mediciones = "ws://10.4.22.146:8000/ws/api/mide/";
const char* websocket_sensores = "ws://10.4.22.146:8000/ws/api/sensores/";
...
void loop() {
  ...
  // Simular datos de luz (0-2998 lux), viento (0-30 m/s) y pH (4.0-8.0)
  ...
  float pHSimulado = random(40, 81) / 10.0; // Rango 4.0-8.0 con un decimal
  ...
  doc["fk_id_sensor"] = 5;
  doc["fk_id_plantacion"] = 1;
  doc["valor_medicion"] = pHSimulado;
  serializeJson(doc, jsonData);
  client_mediciones.send(jsonData);
  Serial.print("📡 Datos enviados por WebSocket (pH): ");
  Serial.println(jsonData);
  doc.clear();

  // Mostrar datos en OLED (sin pH, para no modificar la pantalla)
