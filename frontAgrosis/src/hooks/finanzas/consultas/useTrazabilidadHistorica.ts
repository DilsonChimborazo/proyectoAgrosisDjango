...
const formatDate = (dateStr: string | null | undefined): string => {
    ...
    const date = new Date(dateStr + 'T00:00:00'); // Añade hora para evitar desplazamiento UTC
...
const transformarDatosTrazabilidad = (data: any): TrazabilidadCultivoReporte => ({
    ...
    detalle_herramientas: data.detalle_herramientas?.map((her: any) => ({
        ...her,
        fecha: formatDate(her.fecha),
    })) || [],
    costo_mano_obra_acumulado: data.costo_mano_obra_acumulado ?? 0,
    egresos_insumos_acumulado: data.egresos_insumos_acumulado ?? 0,
    depreciacion_herramientas_acumulada: data.depreciacion_herramientas_acumulada ?? 0,
    ...
    resumen: {
        ...
        total_herramientas: data.resumen?.total_herramientas ?? 0,
    }
});
