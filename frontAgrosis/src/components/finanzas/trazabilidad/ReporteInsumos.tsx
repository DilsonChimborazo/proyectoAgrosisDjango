...
const ReporteInsumos = ({ trazabilidadData, formato }: ReporteInsumosProps) => {
  ...
  const generarReporte = async () => {
    const columnas = ['Insumo', 'Tipo', 'Cantidad', 'Costo Total'];
    ...
    if (formato === 'pdf') {
      ...
      worksheet.getCell('A2').value = `ÁREA PAE - Reporte de Insumos - ${trazabilidadData.cultivo || 'Sin cultivo'}`;
      ...
      datos.forEach((d, index) => {
        const dataRow = worksheet.addRow({ insumo: d[0], tipo: d[1], cantidad: d[2], costo: d[3] });
      ...
      a.href = url;
      a.download = `reporte_insumos_${trazabilidadData.cultivo || 'trazabilidad'}.xlsx`;
