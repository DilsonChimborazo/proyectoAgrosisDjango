...
const ProduccionComponent = ({ showButtons = true }: ProduccionComponentProps) => {
  ...
  const mappedProducciones = produccionesList.map((produccion) => ({
    id: produccion.id,
    nombre_cultivo: produccion.fk_id_plantacion?.fk_id_cultivo?.nombre_cultivo ?? 'Sin cultivo',
        ...
        <VentanaModal
          ...
          contenido={
            <div className="space-y-2 text-sm text-gray-700">
              <p><strong>Cultivo:</strong> {selectedProduccion.fk_id_plantacion?.fk_id_cultivo?.nombre_cultivo ?? 'Sin cultivo'}</p>
