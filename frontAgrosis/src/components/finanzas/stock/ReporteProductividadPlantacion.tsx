...
const ReporteProductividadPlantacion = ({ producciones, formato, onGenerate }: ReporteProductividadPlantacionProps) => {
  ...
  const generarDatos = () => {
    // Filtrar producciones inválidas y loggear para depuración
    ...
    return {
      columnas: ['Producción', 'Unidad', 'Cantidad Producida', 'Stock Disponible', 'Valor Estimado (COP)'],
      datos: datos.map(d => [
        `${d.cultivo} - ${d.nombreProduccion}`,
        d.unidad,
        d.cantidadProducida.toFixed(2),
        d.stock.toFixed(2),
  ...
  const generarReporte = async () => {
    ...
    if (formato === 'pdf') {
      ...
      const worksheet = workbook.addWorksheet('Reporte Productividad Plantacion');
      ...
      a.href = url;
      a.download = 'productividad-plantacion.xlsx';
