...
const ReporteMovimientosStock = ({ allStock, formato, onGenerate }: ReporteMovimientosStockProps) => {
  ...
  const generarDatos = () => {
    // Filtrar movimientos inválidos y loggear para depuración
    ...
    const datos = datosValidos.map(m => ({
      ...
      tipo: m.movimiento,
      cantidad: parseCantidad(m.cantidad),
    ...
    return {
      columnas: ['Cultivo', 'Tipo', 'Cantidad', 'Fecha', 'Unidad'],
      datos: datos.map(d => [
        d.cultivo,
        d.tipo,
        d.cantidad.toFixed(2), // Ahora d.cantidad es garantizadamente un número
        d.fecha,
        d.unidad,
      ]),
      grafico: {
  ...
  const generarReporte = async () => {
    ...
    if (formato === 'pdf') {
      ...
      const worksheet = workbook.addWorksheet('Reporte Movimientos Stock');
